...
public class k_desjie {
		
	public static String desjie(String a,String key)
	{
	...
	public static String desjieda(String big,String key)
	{
		...
		for(int i =0;i<cishu;i++)
		{
			...
			jiemihou = jiemihou + desjie(jiemixiao,key);
		}
		return jiemihou;
	}
	...
	public static void main(String[] args) throws IOException
	{
		...
		//输入初始的明文
		String jie = "" ;
		...
		key  = "";
		System.out.println("key "+key);
		
		String jiemi = desjieda(jie,key);
		System.out.println("jiemihou"+jiemi);
	...
	//置换函数IP,输入明文char型 返回 char型 64位都是
		...
		public static char[] key_gen(char[] Key,int turn)
		{
			...
			for(int i =0;i<48;i++)
			{
				System.out.print(key_converted[i]);
			}
			*/
			//System.out.println();
...
int[][] s8 = new int[][]{
		...
		public static char[] choose(int[] six,int[][] s)
		{
			...
			char[] x = new char[4];
			zhao = s[hang][lie];
			//System.out.print("hanglie"+hang+" "+lie+" zhao"+zhao);
			//System.out.println();
			for(int i = 0;i<4;i++)
			{
				x[i] = int_to_char(binaryToDecimal(zhao),4)[i];
			}
			/*System.out.print("4wei 2jinzhi");
			for(int i = 0;i<4;i++)
			{
		...
		public static char[] p32(char[] x)
		{
			...
			int p32[] = new int[]{16,7,20,21,29,12,28,17,1,15,23,26,5,18,31,10,
